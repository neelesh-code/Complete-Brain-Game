...
var cat, dog, fish, sandwich;

var catImage, dogImage, fishImage, sandwichImage;

var puzzle4Input, puzzle4Try;
...
var clickSound, correctSound, winSound;
...
function preload(){
  ...
  catImage=loadImage("Images/Puzzle4/Cat0.png");
  dogImage=loadImage("Images/Puzzle4/Dog0.png");
  fishImage=loadImage("Images/Puzzle4/Fish0.png");
  sandwichImage=loadImage("Images/Puzzle4/Sandwich0.png");
  smileyImage=loadImage("Images/Logistics/Smiley.png");
...
function setup() {
  ...
  cat=createSprite(150, 300, 50,50);
  ...
  fish=createSprite(750, 300, 50,50);
  fish.addImage(fishImage);
  fish.scale=0.5;
  fish.visible=false;

  sandwich=createSprite(1050, 300, 50,50);
  sandwich.addImage(sandwichImage);
  sandwich.scale=0.5;
  sandwich.visible=false;

  puzzle4Input=createInput("Answer");
  puzzle4Input.hide();

  puzzle4Submit=createButton("Submit")
  puzzle4Submit.hide();

  level5=createSprite(900,500,50,50);
  level5.addImage(level2Image);
  level5.scale=0.35;
  level5.visible=false;

  smiley=createSprite(600,350,50,50);
  smiley.addImage(smileyImage);
  smiley.scale=2;
...
function draw() {
  ...
  if(gameState===4){
    ...
    fill("white");
    textSize(20);
    text("You have a dog, cat, fish, and sandwich. You need to get all these items to the other side of a river, but the boat you are using \n is too small and can only carry one item. If the dog and cat are on one side, then the dog will scare the cat. If the cat is with \nthe fish, then the cat will eat the fish. If the sandwich is with the dog, then the dog will eat the sandwich. However, if the dog is with\n the fish, then the dog will not scare the cat, the dog will not eat the sandwich, and the cat will not eat the fish. How many different ", 30,30);
    text("ways can you put the items through the river?", 435, 130);

    puzzle4Input.position(600,400);
    puzzle4Input.show();

    puzzle4Try=puzzle4Input.value();

    console.log(puzzle4Try);

    puzzle4Submit.position(600, 450);
    puzzle4Submit.size(100,20)
    //puzzle4Submit.show();

    puzzle4Submit.mousePressed(answer4());
  ...
  if(gameState===5){
    ...
    stroke ("white");
    fill ("white")
    text("You Win! The Innocent Boy Is Now Free", 50, 100);
    smiley.visible=true;
